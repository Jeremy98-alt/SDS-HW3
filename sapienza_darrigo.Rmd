...
As pointed out in [[1]](#1), the Pearson correlation coefficient is non-additive, then it is not mathematically meaningful to take the mean of them as they are. Hence, before averaging, we apply the Fisher Z-transform in order to smoothly map the interval $[-1,1]$ into $\mathbb{R}$:
...
$$
C_{12}^{Z(j,k)}\Big(\frac{\alpha}{m}\Big) = \bigg[\overline{Z}_{12}(j,k) \pm z_{\alpha/2m} \cdot \frac{\hat{\sigma}_{j,k}}{\sqrt{12}}\bigg]
$$
...
Show an example of confidence interval between two ROIs, showing its correlation mean:
