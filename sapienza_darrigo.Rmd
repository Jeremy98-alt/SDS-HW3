...
diff.dt$cint <- apply(diff.dt, function(row) {
    list(confidence_interval = c(lb = row$mean.boot - row$se.boot * qnorm(1 - alpha / (2*m)),
                                 ub = row$mean.boot + row$se.boot * qnorm(1 - alpha / (2*m))))
})
```
...
# Estimated graph
G.diff.thr <- graph_from_adjacency_matrix(adj_mat_diff_boot, mode = "undirected")
```
