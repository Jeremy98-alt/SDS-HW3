...
get.est.adj.mat <- function(dt, dec.rule, t) {
  ...
  #     adj_mat[as.character(dt[idx, "ROI_1"]), as.character(dt[idx, "ROI_2"])] <- 1 # dt[idx, "cor.mean"] 
  #   } 
  # }
  
  sapply(1:nrow(dt), function(idx) {
    if( dec.rule(dt, idx, t) ) {
      adj_mat[as.character(dt[idx, "ROI_1"]), as.character(dt[idx, "ROI_2"])] <- 1 # dt[idx, "cor.mean"] 
    } 
  })
