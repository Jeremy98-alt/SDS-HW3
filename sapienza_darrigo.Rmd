...
TODO: derive the correct estimator for the olympic mean; the current confidence interval calculus works only for the sample mean.
...
adj_mat_bonf <- get.est.adj.mat(asd_sel.dt, check.conf.int, z.t) 


# Estimated graph
G.bonf <- graph_from_adjacency_matrix(adj_mat_bonf, mode = "undirected")


# To plot the correlation graph(s)
plot(G.bonf, vertex.size = .5, vertex.color = rgb(0,0,1,.2), label.col = "black", curved = TRUE, 
     main = "Association Graph",  sub = "(Bonferroni's multiplicity adjustment)")
```
...
```{r}
# Adjacency matrix of the estimated graph with the Bonferroni's correction
adj_mat_thr <- get.est.adj.mat(asd_sel.dt, check.threshold, z.t) 


# Estimated graph
G.thr <- graph_from_adjacency_matrix(adj_mat_thr, mode = "undirected")


plot(G.thr, vertex.size = .5, vertex.color = rgb(0,0,1,.2), label.col = "black", curved = TRUE, 
     main = "Association Graph",  sub = "(Threshold only)")
```
...
# Adjacency matrix of the estimated graph with the Bonferroni's correction
adj_mat_no_bonf <- get.est.adj.mat(asd_sel.dt, check.conf.int, z.t) 


# Estimated graph
G.no.bonf <- graph_from_adjacency_matrix(adj_mat_no_bonf, mode = "undirected")


plot(G.no.bonf, vertex.size = .5, vertex.color = rgb(0,0,1,.2), label.col = "black", curved = TRUE, 
     main = "Association Graph",  sub = "(Without Bonferroni's multiplicity adjustment)")
```
...
```{r}
paste("Number of edges considering only the threshold t:", gsize(G.thr))
paste("Number of edges with Bonferroni's correction:", gsize(G.bonf))
paste("Number of edges without Bonferroni's correction:", gsize(G.no.bonf))
```
