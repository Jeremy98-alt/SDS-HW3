...
get.cor.ROIs <- function(...){
  ...
  corr.ROIs$z.pearson.corr <- apply(corr.ROIs, 1, function(row) {
    atanh(cor(args[["frame"]][row["ROI_1"]], args[["frame"]][row["ROI_2"]])) # takes the sets of columns; apply the Z-transform to the
                                                          # correlation matrix
  })
...
non.parametric.bootstrap <- function(data1, data2, B = 1000) {
  p.samples <- abs(get.cor.ROIs(frame = data1)$z.pearson.corr - get.cor.ROIs(frame = data2)$z.pearson.corr)
  n <- length(p.samples)
  p.boot <- array(rep(matrix(NA, nrow = n, ncol = 1), B))
  
  for(b in 1:B) {
    idx <- sample(1:n, n, replace = TRUE)
    p.boot[b] <- abs(
      as.matrix(
      get.cor.ROIs(frame = data1[idx,])$z.pearson.corr - get.cor.ROIs(frame = data2[idx,])$z.pearson.corr
      )
    )
  }
...
alpha <- 0.05 # 95%
m <- (116 * 115) / 2
mean.boot <- apply(out.boot$p.boot, 1:2, mean)
se.boot <- apply(out.boot$p.boot, 1:2, sd)

out.boot$cint <- sapply(data, function(z.i) {
    list(confidence_interval = c(lb = z.i - se * qnorm(1 - alpha / (2*m)), 
                                 ub = z.i + se * qnorm(1 - alpha / (2*m))))
})
```
...
out.boot <- non.parametric.bootstrap(td_fusion, B = 1000) 
