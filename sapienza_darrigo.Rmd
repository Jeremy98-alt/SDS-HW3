...
Now, we find the quantile value for these two groups. Sincewe are working in the Z-scale, the threshold is also in the Z-scale.
...
Even though the most frequent degree is the same for both groups, there are some differences in the degree distribution of the two graphs. From this, we deduce that some ROIs have different co-activation relations between in ASD and TD patients.
...
# call the creation of unique matrix
asd_sel.dt <- unique.matrix(asd_sel)
td_sel.dt <- unique.matrix(td_sel)
```
...
mean.diff <- abs(asd_sel.dt[, 3] - td_sel.dt[, 3]) # difference between the z-correlation means
...
This time, since we are testing $\Big|{z}^{ASD}_{j,k} - {z}^{TD}_{j,k}\Big| \geq z_t$, we assume as a conservative threshold the $80^{th}$ percentile of this difference: within $[-z_t, z_t]$, $z_{j,k}$ is considered equal in both the groups.

$$~$$

```{r}
z.diff.t <- apply(diff.dt["cor.mean"], 2, quantile, probs=0.8) 
paste("The threshold at the 40th percentile for the difference is: ", round(as.numeric(z.diff.t), 3), sep = "")
```

$$~$$

We check the ECDF of the this difference:
...
non.parametric.bootstrap <- function(data1, data2, B = 1000) {
  p.samples <- abs(atan(cor(data1)) - atan(cor(data2))) # we want to have the difference sample between our two correlation measures
  ...
  for(b in 1:B) {
    ...
    p.boot[b,,] <- abs(atan(cor(data1[idx1,])) - atan(cor(data2[idx2,]))) # inserting the i-th estimated pearson's correlation
  }
...
out.boot <- non.parametric.bootstrap(asd_fusion, td_fusion, B = 1000) # getting the bootstrapped correlations samples
...
diff.dt$cint <- sapply(1:nrow(diff.dt), function(idx) { # inserting the estimated CI boostrapped
