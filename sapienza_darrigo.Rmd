...
<br><br>
![](./sapienza_darrigo_files/figure-html/data_tree.png)
...
```{r}
hist(asd_sel.dt$olymp.mean, probability = T, breaks = 50, col = "yellow", main = "Histogram of ASD group", xlab = "olympic mean", ylab = "count", border = "white")
```

$$~$$

```{r}
hist(td_sel.dt$olymp.mean, probability = T, breaks = 50, col = "blue", main = "Histogram of TD group", xlab = "olympic mean", ylab = "count", border = "white")
```
...
plotCint <- function(ROI.1, ROI.2, dt) {
  hist(dt$olymp.mean, probability = T, breaks = 50, col = "pink", main = "Interval of a data point", xlab = "olympic mean", ylab = "count")
...
olymp.diff <- asd_sel.dt[, 3] - td_sel.dt[, 3]
...
hist(p.boot, prob = TRUE, main = "Correlation pearson", xlab = "Pearson :: Bootstrap replicates", col  = "orchid", border = "purple", breaks=50)
rug(p.boot); box()

# Bias
paste("The bias is equal to: ", round(mean(p.boot) - mean(p.samples), 3), sep="")

# Standard deviation
paste("The standard deviation is: ", round(sd(p.boot), 3), sep="")

# Normal Bootstrap CI
alpha <- 0.05 # 90%

low <- mean(p.boot) - qnorm(1-alpha/2)*sd(p.boot) 
up <- mean(p.boot) + qnorm(1-alpha/2)*sd(p.boot) 
