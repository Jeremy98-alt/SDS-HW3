...
Start to find the quantile values for these two groups.

```{r}
# pick through all correlation values the 80th percentile
t.asd_sel <- apply(asd_sel.dt["olymp.mean"], 2, quantile, probs=0.8) 
paste("the threshold at 80% percentile for the ASD sujects is: ", round(as.numeric(t.asd_sel), 3), sep = "")
...
After that, We decide to construct the adjacency matrices 
